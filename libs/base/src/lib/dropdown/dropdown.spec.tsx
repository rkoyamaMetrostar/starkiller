...
describe('Dropdown', () => {
  ...
  it('should change selected option to first real option', () => {
    const { baseElement } = render(
      <Dropdown id={defaultId} name={defaultName} options={options} />
    );
    ...
    userEvent.selectOptions(selectElement, options[0].value as string);
    expect(
      baseElement.querySelector('option:checked')?.getAttribute('value')
    ).toEqual(options[0].value);
  });

  it('changing option should trigger onChanged event handler', () => {
